    ...
    public class MFConfigHelper : IDisposable
    {
        ...
        private void WriteConfig(string configName, byte[] data, bool staticSize, bool updateConfigSector)
        {
            ...
            else        // adding a new configuration to the end of the current list
            {
                ...
                // martinca: fix allocation - was too small : byte[] temp = new byte[m_lastCfgIndex >= m_all_cfg_data.Length ? m_lastCfgIndex + data.Length : m_all_cfg_data.Length];
                byte[] temp = new byte[newLastIndex >= m_all_cfg_data.Length ? m_lastCfgIndex + data.Length : m_all_cfg_data.Length];
